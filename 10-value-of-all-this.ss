...
; The looup-in-entry function looks in an entry to find the value by name
;
...
; A table (alse called an environment) is a list of entries.
...
; The extend-table function takes an entry and a table and adds entry to the table
;
...
(lookup-in-table
 ...
 '(((entree dessert)(spaghetti spumoni))
   ((appetizer entree bevarage)(food tastes good)))
 (lambda(n)'()))
; ==> 'good
...
; evlis finds meaning od arguments
;
(define evlis
  (lambda(args table)
    (cond
