...
; The eq?-c function takes an atom and returns a function that takes
; an atom and tests if they are the same
;
(define eq?-c
  (lambda (a)
    (lambda (x)
...
; insert-g acts as insertL or insertR depending on the helper function passed to it
;
(define insert-g
  (lambda (seq)
    (lambda (new old l)
...
; subst is now just (insert-g seqS)
;
...
;                                                                       ;
; Abstract comman patterns with a new function.                         ;
;                                                                       ;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
;
(atom-to-function (operator '(o+ 5 3)))    ; #<procedure:+>

; The value function rewrittrn to use abstraction
;
(define value
  (lambda (nexp)
    (cond
...
; Example of multirember&co with the new friend function
;
(multirember&co
 'tuna
 '(strawberries tuna with and swordfish)
 new-friend)
; ==> #f
...
; Examples of multirember&co with the last friend function
;
(multirember&co
 'tuna
 '(strawberries tuna and swordfish)
...
;                                                                       ;
; Build functions to collect more than one value                        ;
; at a time.                                                            ;
;                                                                       ;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

; The multiinsertR inserts to the left and to the right of elements
;
(define multiinsertR
...
; evnes-product-friend returns the product of evens
;
(define evens-product-friend
  (lambda (e p s)
    p))

; Example of evens-product-friend
;
(evens-only*&co
 '((9 1 2 8) 3 10 ((9 9) 7 6) 2)
