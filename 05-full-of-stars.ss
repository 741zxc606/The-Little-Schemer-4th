...
(rember*
 'sauce
 '(((tomato sauce))((bean)sauce)(and ((flying) sauce))))    ; '(((tomato)) ((bean)) (and ((flying))))

; The insertR* function inserts new to the right of all olds in l
;
...
; And when recurring on a list if S-expressions, l, use (car l) and (cdr l) if neither        ;
...
; The member* function  determines if element is a list l of S-expressions
;
(define member*
  (lambda(a l)
    (cond
...
(define eqlist2?
  (lambda (l1 l2)
    (cond
      ; case 1: l1 isempty, l2 is empty, atom, list
      ((and (null? l1)(null? l2))#t)
...
; rember function simplified, it now also works on S-expressions, not just atoms
;
(define rember
  (lambda (s l)
    (cond
